...
public class ImmoESI {
    ...
    public void afficherListBiens() {
        ...
        for(Bien bien : listBiens) {
            System.out.println("  "+bien.getIdentifiant()+"   "+bien.getClass().getSimpleName()+" "+bien.getDateAjoutString());
        }
    }
    ...
    public Bien fetchBien(int id) throws NullPointerException {
        if(isAuthenticated) {
            for (Bien bien : ImmoESI.listBiens) {
                if (bien.getIdentifiant() == id) return bien;
            }
        } else {
    ...
    public boolean archiveBien(Bien b, Proprietaire proprietaire) {
        if(isAuthenticated) {
            if (removeBien(b, proprietaire)) return listArchive.add(b);
            else {
    ...
    public void afficherListBiensProp(Proprietaire p) {
        if(isAuthenticated && p != null) p.afficherListBiens();
        ...
        if(isAuthenticated) {
            ...
            if (listProprietaires.contains(p)) {
                for (Bien b : listBiens) {
                    if (b.getProprietaire().equals(p)) removeBien(b, p);
                }
    ...
    public void filter(Transaction transaction,
        ...
        for(Bien b : listBiens){
            if (b.getNatureTransaction()==criteriaMap.get("type_transaction") || criteriaMap.get("type_transaction")==null)
                if(b.getWilaya()==criteriaMap.get("wilaya") || criteriaMap.get("wilaya")==null)
                    if(b.getPrixFinal()<=(double)criteriaMap.get("prix_max") || (double)criteriaMap.get("prix_max")==-1)
                        if(b.getPrixFinal()>=(double)criteriaMap.get("prix_min") || (double)criteriaMap.get("prix_min")==-1)
                            if(b.getClass().getSimpleName().equals(criteriaMap.get("type_bien").toString()) || criteriaMap.get("type_bien").equals(""))
                                if(b.getSuperficie()>=(float)criteriaMap.get("superficie_min") || (float)criteriaMap.get("superficies_min")==-1)
                                    if((((Habitable) b).getNbPieces()>=(int)criteriaMap.get("nbpieces_min"))|| (int)criteriaMap.get("nbpieces_min")==-1)
                                        b.afficherInfo();
        }
    }
    /*
    public void filtrer() {
        ...
        while (!stop) {
            ...
            if (choix < 1 || choix > 7) {
                ...
                switch (choix) {
                    case 1:
                        System.out.print("choisir type de transaction\n\n");
                    ...
                    case 3:
                        ...
                        prixMinimum = sc.nextDouble();
                        break;
                    case 4:
                        System.out.print("entrer le prix maxximum\n\n");
                        prixMaximum = sc.nextDouble();
                        break;
                    case 5:
                        System.out.print("choisir type de bien\n\n");
                        System.out.print("Maison\n");
                        System.out.print("Terrain\n");
                        System.out.print("Appartement\n");
                        typeBien = sc.nextLine();
                        break;
                    case 6:
        ...
        for (Bien b : listBiens) {
            if (b.getNatureTransaction().getNomTrans().compareTo(transaction) == 0 || transaction.equals(""))
                if (b.getWilaya().compareTo(wilaya) == 0 || wilaya.equals(""))
                    if (b.getPrixFinal() >= prixMinimum || prixMinimum == 0)
                        if (b.getPrixFinal() <= prixMaximum || prixMaximum == 0)
                            if (b.getClass().toString().compareTo(typeBien) == 0 || typeBien.equals(""))
                                if (b.superficie >= superficieMin || superficieMin == 0)
                                    b.afficherInfo();
